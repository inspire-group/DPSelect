...
shannon_decrease =  open('min_shannon_decreaseCRTille','w+')
ases_to_ips = {}
ips_to_bandwidthN= {}
ips_to_bandwidth = {}
ip_weighted_prob = {}
	...
	for ip in as_to_ip_probability[key]:
		for index,weight in enumerate(as_to_ip_probability[key][ip]):
			...
			if to_as in as_to_as_probability[key]:
				...
				as_to_as_probability[key][to_as] = Decimal(as_to_ip_probability[key][ip][index])
for actual_source_as in as_to_as_probability:
	...
	for j in range(0,100):
		...
		for i in range(0,100):
			...
			if len(entropies) <100:
				...
				entropies[i]+=entropy
	for h in range(0,len(entropies)):
		entropies[h] = entropies[h]/100
	print(entropies[99])
	shannon_decrease.write(actual_source_as+"\n")
...
highest_as = 6713
entropy = 0
d = as_to_as_probability["6713"]
lowest = 1
highest = 0
for as1 in as_to_as_probability:
	total_probability+=as_to_as_probability[as1]["16276"]
	if as_to_as_probability[as1]["16276"] < lowest:
		lowest = as_to_as_probability[as1]["16276"]
		lowest_as = as1
	if as_to_as_probability[as1]["16276"] > highest:
		highest = as_to_as_probability[as1]["16276"]
		highest_as = as1
for as1 in as_to_as_probability:
	entropy+= -as_to_as_probability[as1]["16276"]/total_probability*math.log(as_to_as_probability[as1]["16276"]/total_probability,2)
print(entropy)
...
index = 0
total = 0
# Sort list of ips by bandwidth
s = [(k, ips_to_bandwidthNormal[k]) for k in sorted(ips_to_bandwidthNormal, key=ips_to_bandwidthNormal.get, reverse=True)]
for ip in as_to_ip_probability["13184"]:
	if s[index][0] in as_to_ip_probability["13184"]:
		if (s[index][1] > 0):
			x.append(s[index][0])
			y.append(as_to_ip_probability["13184"][s[index][0]][0])
			total+=as_to_ip_probability["13184"][s[index][0]][0]
	index = index+1
print(total)
# I need to sort this probability distribution by the the bandwidth of relay


while total_probability < .99999:
	total_probability= 0
	for ip in as_to_ip_probability["20001"]:
		for index,weight in enumerate(as_to_ip_probability[key][ip]):
			if ips_to_bandwidth[ip] < current_bandwidth:
				total_probability+=as_to_ip_probability[key][ip][index]
	x.append(current_bandwidth)
	y.append(total_probability)
	print(y)
	current_bandwidth+=100

#for a in as_to_ip_probability:
#	print(a)

x = as_to_ip_probability["20001"]
y = []
for ip in x:
	if (x[ip][0] != 0 ):
		y.append(x[ip][0])
...
std = 0
for num in y:
	std +=math.pow(abs(num-avg),2)
std =math.pow( std/len(y),.5)
print(std)
'''