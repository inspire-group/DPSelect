...
shannon_decrease =  open('shannon_decreaseOct5432UDP','w+')
ases_to_ips = {}
ips_to_bandwidthN= {}
ips_to_bandwidth = {}
ip_weighted_prob = {}
...
for j in range(0,1000):
	...
	for i in range(0,50):
		...
		for source_as in shannon_probabilities:
			ctx = Context(prec=40)
			two = Decimal(2)
			entropy+= -Decimal(shannon_probabilities[source_as])*ctx.divide(Decimal(shannon_probabilities[source_as]).ln(ctx), two.ln(ctx))
